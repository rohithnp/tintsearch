...
- Throw `Tintsearch::ImportError` for errors when importing records
- Errors now inherit from `Tintsearch::Error`
...
- Raise `Tintsearch::DangerousOperation` error when calling reindex with scope
...
- Made `Tintsearch.disable_callbacks` thread-safe
...
- Added error if `tintsearch` is called multiple times
