...
Tintsearch learns what **your users** are looking for.  As more people search, it gets smarter and the results get better.  It’s friendly for developers - and magical for your users.
...
**Tintsearch 1.0 was just released!** See [instructions for upgrading](#100)
...
[![Build Status](https://travis-ci.org/ankane/tintsearch.svg?branch=master)](https://travis-ci.org/ankane/tintsearch)
...
For Elasticsearch 2.0, use the version `1.0` and above. For Elasticsearch 0.90, use version `0.6.3` and [this readme](https://github.com/ankane/tintsearch/blob/v0.6.3/README.md).

Add tintsearch to models you want to search.
...
Tintsearch supports the complete [Elasticsearch Search API](https://www.elastic.co/guide/en/elasticsearch/reference/current/search-search.html). As your search becomes more advanced, we recommend you use the [Elasticsearch DSL](#advanced) for maximum flexibility.
...
Searches return a `Tintsearch::Results` object. This responds like an array to most methods.
...
Tintsearch defaults to English for stemming.  To change this, use:

```ruby
class Product < ActiveRecord::Base
  tintsearch language: "german"
end
```
...
```ruby
class Product < ActiveRecord::Base
  tintsearch synonyms: [["scallion", "green onion"], ["qtip", "cotton swab"]]
  # or
  # tintsearch synonyms: -> { CSV.read("/some/path/synonyms.csv") }
end
```
...
```ruby
class Product < ActiveRecord::Base
  tintsearch wordnet: true
end
```

### Misspellings

By default, Tintsearch handles misspelled queries by returning results with an [edit distance](https://en.wikipedia.org/wiki/Levenshtein_distance) of one.
...
To improve performance for correctly spelled queries (which should be a majority for most applications), Tintsearch can first perform a search without misspellings, and if there are too few results, perform another with them.
...
Tintsearch uses `find_in_batches` to import documents.  To eager load associations, use the `search_import` scope.
...
- when you install or upgrade tintsearch
- change the `search_data` method
- change the `tintsearch` method
...
2. Asynchronous
  ...
  ```ruby
  class Product < ActiveRecord::Base
    tintsearch callbacks: :async
  end
  ```

  And [install Active Job](https://github.com/ankane/activejob_backport) for Rails 4.1 and below. Jobs are added to a queue named `tintsearch`.
...
Tintsearch can use conversion data to learn what users are looking for.  If a user searches for “ice cream” and adds Ben & Jerry’s Chunky Monkey to the cart (our conversion metric at Instacart), that item gets a little more weight for similar searches.
...
You do **not** need to clean up the search queries.  Tintsearch automatically treats `apple` and `APPLES` the same.
...
class Product < ActiveRecord::Base
  ...
  tintsearch conversions: "conversions" # name of field
...
```ruby
rake tintsearch:reindex CLASS=Product
```
...
![Autocomplete](http://ankane.github.io/tintsearch/autocomplete.png)

**Note:** If you only have a few thousand records, don’t use Tintsearch for autocomplete. It’s *much* faster to load all records into JavaScript and autocomplete there (eliminates network requests).

First, specify which fields use this feature.  This is necessary since autocomplete can increase the index size significantly, but don’t worry - this gives you blazing faster queries.

```ruby
class Book < ActiveRecord::Base
  tintsearch match: :word_start, searchable: [:title, :author]
end
```
...
![Suggest](http://ankane.github.io/tintsearch/recursion.png)

```ruby
class Product < ActiveRecord::Base
  tintsearch suggest: [:name] # fields to generate suggestions
end
```
...
![Aggregations](http://ankane.github.io/tintsearch/facets.png)
...
Facets have been deprecated in favor of aggregations as of Tintsearch 1.0. See [how to upgrade](#moving-from-facets).
...
Tintsearch supports [Elasticsearch’s routing feature](https://www.elastic.co/blog/customizing-your-document-routing).

```ruby
class Business < ActiveRecord::Base
  tintsearch routing: true

  def tintsearch_routing
    city_id
  end
end
```
...
Tintsearch supports single table inheritance.
...
```ruby
Product.tintsearch_index.tokens("Dish Washer Soap", analyzer: "default_index")
# ["dish", "dishwash", "washer", "washersoap", "soap"]

Product.tintsearch_index.tokens("dishwasher soap", analyzer: "tintsearch_search")
# ["dishwashersoap"] - no match

Product.tintsearch_index.tokens("dishwasher soap", analyzer: "tintsearch_search2")
# ["dishwash", "soap"] - match!!
```
...
```ruby
Product.tintsearch_index.tokens("San Diego", analyzer: "tintsearch_word_start_index")
# ["s", "sa", "san", "d", "di", "die", "dieg", "diego"]

Product.tintsearch_index.tokens("dieg", analyzer: "tintsearch_word_search")
# ["dieg"] - match!!
```

See the [complete list of analyzers](lib/tintsearch/index.rb#L209).

## Deployment

Tintsearch uses `ENV["ELASTICSEARCH_URL"]` for the Elasticsearch server.  This defaults to `http://localhost:9200`.
...
```sh
heroku run rake tintsearch:reindex CLASS=Product
```
...
```ruby
Tintsearch.client = Elasticsearch::Client.new(hosts: ["localhost:9200", "localhost:9201"], retry_on_failure: true)
```
...
config.lograge.custom_options = lambda do |event|
  options = {}
  options[:search] = event.payload[:tintsearch_runtime] if event.payload[:tintsearch_runtime].to_f > 0
  options
end
```
...
To keep the mappings and settings generated by Tintsearch, use:

```ruby
class Product < ActiveRecord::Base
  tintsearch merge_mappings: true, mappings: {...}
end
```
...
To modify the query generated by Tintsearch, use:
...
Tintsearch.multi_search([fresh_products, frozen_products])
```
...
# or this ...
Product.tintsearch_index.import(some_company.products)
...
Product.where("id > 100000").find_in_batches do |batch|
  Product.tintsearch_index.import(batch)
end
```
...
```ruby
class Product < ActiveRecord::Base
  tintsearch settings: {number_of_shards: 3}
end
```
...
```ruby
class Product < ActiveRecord::Base
  tintsearch index_name: "products_v2"
end
```
...
```ruby
class Product < ActiveRecord::Base
  tintsearch index_name: -> { "#{name.tableize}-#{I18n.locale}" }
end
```
...
```ruby
class Product < ActiveRecord::Base
  tintsearch index_prefix: "datakick"
end
```
...
```ruby
Tintsearch.timeout = 15 # defaults to 10
```

Set a lower timeout for searches

```ruby
Tintsearch.search_timeout = 3
```

Change the search method name in `config/initializers/tintsearch.rb`

```ruby
Tintsearch.search_method_name = :lookup
```
...
class Product < ActiveRecord::Base
  # A will not match Ä
  tintsearch special_characters: false
end
```
...
```ruby
class Product < ActiveRecord::Base
  tintsearch batch_size: 200 # defaults to 1000
end
```
...
```ruby
Tintsearch.search "milk", index_name: [Product, Category]
```

Reindex all models - Rails only

```sh
rake tintsearch:reindex:all
```
...
For large data sets, check out [Keeping Elasticsearch in Sync](https://www.elastic.co/blog/found-keeping-elasticsearch-in-sync).  Tintsearch will make this easy in the future.
...
describe Product do
  it "searches" do
    Product.reindex
    Product.tintsearch_index.refresh # don't forget this
    # test goes here...
  end
end
```
...
Product.tintsearch_index.refresh # or this
```
...
2. Replace tire mapping w/ tintsearch method
...
3. Deploy and reindex
  ...
  ```ruby
  rake tintsearch:reindex CLASS=Product # or Product.reindex in the console
  ```

4. Once it finishes, replace search calls w/ tintsearch calls

## Upgrading

View the [changelog](https://github.com/ankane/tintsearch/blob/master/CHANGELOG.md).
...
- **ActiveRecord 4.1+ and Mongoid 3+:** Attempting to reindex with a scope now throws a `Tintsearch::DangerousOperation` error to keep your from accidentally recreating your index with only a few records.
...
If running Tintsearch `0.6.0` or `0.7.0` and Elasticsearch `0.90`, we recommend upgrading to Tintsearch `0.6.1` or `0.7.1` to fix an issue that causes downtime when reindexing.
...
```ruby
class Product < ActiveRecord::Base
  tintsearch settings: {number_of_shards: 1}
end
```
...
- [Report bugs](https://github.com/ankane/tintsearch/issues)
- Fix bugs and [submit pull requests](https://github.com/ankane/tintsearch/pulls)
...
```sh
git clone https://github.com/ankane/tintsearch.git
cd tintsearch
bundle install
rake test
```
