module Tintsearch
  class Query
    ...
    def initialize(klass, term, options = {})
      ...
      @term = term
      @options = options
      @match_suffix = options[:match] || tintsearch_options[:match] || "analyzed"

      prepare
    end

    def tintsearch_index
      klass ? klass.tintsearch_index : nil
    end

    def tintsearch_options
      klass ? klass.tintsearch_options : {}
    end

    def tintsearch_klass
      klass ? klass.tintsearch_klass : nil
    end

    def params
      index =
        if options[:index_name]
          Array(options[:index_name]).map { |v| v.respond_to?(:tintsearch_index) ? v.tintsearch_index.name : v }.join(",")
        elsif tintsearch_index
          tintsearch_index.name
        else
          "_all"
        end
    ...
    def handle_response(response)
      ...
      @execute = Tintsearch::Results.new(tintsearch_klass, response, opts)
    end
    ...
    def handle_error(e)
      ...
      if status_code == 404
        ...
        raise UnsupportedVersionError, "This version of Tintsearch requires Elasticsearch 1.0 or greater"
      elsif status_code == 400
        if e.message.include?("[multi_match] analyzer [tintsearch_search] not found")
    ...
    def reindex_command
      tintsearch_klass ? "#{tintsearch_klass.name}.reindex" : "reindex"
    end

    def execute_search
      Tintsearch.client.search(params)
    end

    def prepare
      boost_fields = {}
      fields = options[:fields] || tintsearch_options[:searchable]
      fields =
        if fields
          if options[:autocomplete]
            ...
            fields.map do |value|
              k, v = value.is_a?(Hash) ? value.to_a.first : [value, options[:match] || tintsearch_options[:match] || :word]
          ...
          if options[:autocomplete]
            (tintsearch_options[:autocomplete] || []).map { |f| "#{f}.autocomplete" }
          else
            ["_all"]
          end
        end
      ...
      conversions_field = tintsearch_options[:conversions]
      personalize_field = tintsearch_options[:personalize]
      ...
      if options[:json]
        ...
        # suggestions
        if options[:suggest]
          suggest_fields = (tintsearch_options[:suggest] || []).map(&:to_s)
        ...
        if options[:type] || (klass != tintsearch_klass && tintsearch_index)
          @type = [options[:type] || klass].flatten.map { |v| tintsearch_index.klass_document_type(v) }
        end
    ...
    def below_version?(version)
      Gem::Version.new(Tintsearch.server_version) < Gem::Version.new(version)
    end
  end
end
