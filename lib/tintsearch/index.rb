module Tintsearch
  class Index
    ...
    def bulk_delete(records)
      Tintsearch.queue_items(records.reject { |r| r.id.blank? }.map { |r| {delete: record_data(r)} })
    end

    def bulk_index(records)
      Tintsearch.queue_items(records.map { |r| {index: record_data(r).merge(data: search_data(r))} })
    end
    ...
    def reindex_record_async(record)
      if Tintsearch.callbacks_value.nil?
        if defined?(Tintsearch::ReindexV2Job)
          Tintsearch::ReindexV2Job.perform_later(record.class.name, record.id.to_s)
        else
          Delayed::Job.enqueue Tintsearch::ReindexJob.new(record.class.name, record.id.to_s)
        end
      else
    ...
    def search_model(tintsearch_klass, term = nil, options = {}, &block)
      query = Tintsearch::Query.new(tintsearch_klass, term, options)
    ...
    def create_index(options = {})
      ...
      index = Tintsearch::Index.new("#{name}_#{Time.now.strftime('%Y%m%d%H%M%S%L')}", @options)
    ...
    def clean_indices
      ...
      indices.each do |index|
        Tintsearch::Index.new(index).delete
      end
      indices
    end
    ...
    def reindex_scope(scope, options = {})
      ...
      index = create_index(index_options: scope.tintsearch_index_options)
          ...
          if !options[:searchable] || mapping_options[:searchable].include?(field)
            ...
            mapping_options.except(:highlight, :searchable, :only_analyzed).each do |type, fields|
              if options[:match] == type || fields.include?(field)
                field_mapping[:fields][type] = {type: "string", index: "analyzed", analyzer: "tintsearch_#{type}_index"}
              end
            end
          end
    ...
    def search_data(record)
      ...
      options = record.class.tintsearch_options
