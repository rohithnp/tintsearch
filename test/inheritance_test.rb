...
class InheritanceTest < Minitest::Test
  def test_child_reindex
    store_names ["Max"], Cat
    assert Dog.reindex
    Animal.tintsearch_index.refresh
    assert_equal 1, Animal.search("*").size
  end

  def test_child_index_name
    assert_equal "animals-#{Date.today.year}", Dog.tintsearch_index.name
  end
  ...
  def test_multiple_indices
    ...
    assert_search "product", ["Product A", "Product B"], index_name: [Product.tintsearch_index.name, Animal.tintsearch_index.name], conversions: false
  end
end
