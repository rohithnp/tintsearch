...
class IndexTest < Minitest::Test
  def test_clean_indices
    old_index = Tintsearch::Index.new("products_test_20130801000000000")
    different_index = Tintsearch::Index.new("items_test_20130801000000000")
    ...
    assert Product.tintsearch_index.exists?
    assert different_index.exists?
    assert !old_index.exists?
  end

  def test_clean_indices_old_format
    old_index = Tintsearch::Index.new("products_test_20130801000000")
    old_index.create
  ...
  def test_tokens
    assert_equal ["dollar", "dollartre", "tree"], Product.tintsearch_index.tokens("Dollar Tree")
  end

  def test_tokens_analyzer
    assert_equal ["dollar", "tree"], Product.tintsearch_index.tokens("Dollar Tree", analyzer: "tintsearch_search2")
  end
  ...
  def test_bad_mapping
    Product.tintsearch_index.delete
    store_names ["Product A"]
    assert_raises(Tintsearch::InvalidQueryError) { Product.search "test" }
  ensure
    Product.reindex
  end
  ...
  def test_remove_blank_id
    store_names ["Product A"]
    Product.tintsearch_index.remove(OpenStruct.new)
  ...
  def test_missing_index
    assert_raises(Tintsearch::MissingIndexError) { Product.search "test", index_name: "not_found" }
  end

  def test_unsupported_version
    raises_exception = ->(_) { raise Elasticsearch::Transport::Transport::Error.new("[500] No query registered for [multi_match]") }
    Tintsearch.client.stub :search, raises_exception do
      assert_raises(Tintsearch::UnsupportedVersionError) { Product.search("test") }
    end
  end

  def test_invalid_query
    assert_raises(Tintsearch::InvalidQueryError) { Product.search(query: {boom: true}) }
  end
