...
class DangerousReindexTest < Minitest::Test
  ...
  def test_dangerous_reindex
    assert_raises(Tintsearch::DangerousOperation) { Product.where(id: [1, 2, 3]).reindex }
  end

  def test_dangerous_index_associations
    Store.create!(name: "Test")
    assert_raises(Tintsearch::DangerousOperation) { Store.first.products.reindex }
  end
  ...
  def test_dangerous_reindex_inheritance
    assert_raises(Tintsearch::DangerousOperation) { Dog.where(id: [1, 2, 3]).reindex }
  end
end
